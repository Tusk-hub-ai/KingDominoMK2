...
path_input = Path(__file__).parent / "mine+swamp"
path_output = Path(__file__).parent / "crown"
lis = []
crown_size = 29
half = crown_size // 2
...
def mouseEvent(event, x , y ,flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        path = path_output / f"{current_path[0].stem}_crown_{crown_counter[0]}.png"
...
lis = sorted(path_input.iterdir(), key=lambda f: int(f.stem.split('_')[0]))
